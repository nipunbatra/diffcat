...
// advances differ from pdf-lib's by up to 0.06 em per glyph pair.)
{
  ...
  // the fixture only proves something if MuPDF itself welds the words
  const check = mupdf.Document.openDocument(fs.readFileSync(path.join(DIR, "tightv2.pdf")), "application/pdf");
  const raw = check.loadPage(0).toStructuredText().asText();
  if (!/[a-z][A-Z]|brownfox|overthe|withoutany/.test(raw.replace(/\s/g, "")) && raw.split(" ").length > 20) {
    console.log("note: this MuPDF build spaced tightv2.pdf on its own; the tight fixture is not exercising the fallback");
  }
}
